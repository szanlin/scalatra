...
trait FileUploadSupport extends ServletBase {
  ...
  override def handle(req: ServletHttpRequest, resp: ServletHttpResponse) {
    val req2 = try {
  ...
  */
  private def isMultipartContent(req: ServletHttpRequest) = {
  ...
  private def extractMultipartParams(req: ServletHttpRequest): BodyParams =
