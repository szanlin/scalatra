...
//  }
//
//  def status = ResponseStatus(underlying.getStatus.getCode, underlying.getStatus.getReasonPhrase)
//  def status_=(newStatus: ResponseStatus) = underlying.setStatus(new HttpResponseStatus(newStatus.code, newStatus.message))
//
//  // Why is this an option? it should always have a value
//  def contentType = headers.get(Names.CONTENT_TYPE).flatMap(_.blankOption)
//
//
